...
from .main_window_widgets import CommandDockWidget, DatasetWidget, LogWidget, \
    PreferencesDialog, AboutDialog, LicenseDialog
...
class MainWindow(QMainWindow):
    ...
    def setup_log_ui(self):
        ...
        # Python Log
        python_log_widget = LogWidget(parent=self, filetype="Python Files (*.py)", initial_log=["import clarite\n\n"])
        python_log_widget.btn_clear.setHidden(True)  # Don't allow the python log to be cleared- too tricky
    ...
    def setup_menu(self):
        ...
        preferences_action.triggered.connect(lambda: PreferencesDialog(parent=self.appctx.main_window).show())
        ...
        dock_commands_action.triggered.connect(
            lambda make_floating: self.command_dock_widget.setFloating(not make_floating))
        ...
        showAboutButton.triggered.connect(lambda: AboutDialog(parent=self.appctx.main_window).show())
        ...
        showLicenseButton.triggered.connect(lambda: LicenseDialog(parent=self.appctx.main_window).show())
