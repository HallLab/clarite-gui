...
class ManhattanPlotDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.cutoff_bonferroni_sb.valueChanged.connect(
            self.update_bonferroni_cutoff_value
        )
    ...
    def update_dataset_selection(self):
        ...
        self.datasets = {
            d.name: d.df for d in self.appctx.datasets if d.name in selected_names
        }
        ...
        if len(self.datasets) == 0:
            ...
            self.datasets_label.setText(
                f"1 Dataset Selected ({len(self.variables):,} variables)"
            )
            self.label_specific_btn.setEnabled(True)
            self.update_plot_btn.setEnabled(True)
        else:
            self.datasets_label.setText(
                f"{len(self.datasets):,} Datasets Selected ({len(self.variables):,} variables)"
            )
            self.label_specific_btn.setEnabled(True)
            self.update_plot_btn.setEnabled(True)
    ...
    def launch_get_category_file(self):
        ...
        try:
            ...
            show_warning(
                "Variable Categories File Error", f"Error reading file: {str(e)}"
            )
            return
    ...
    def launch_get_specific(self):
        ...
        specific = SelectColumnDialog.get_column(
            columns=self.variables, selected=self.label_specific, parent=self
        )
