...
class RowfilterIncompleteDialog(QDialog):
    """
    This dialog sets settings for colfilter percent zero
    """
    def __init__(self, *args, **kwargs):
        super(RowfilterIncompleteDialog, self).__init__(*args, **kwargs)
    ...
    def log_command(self):
        ...
        self.appctx.log_python(f"{new_data_name} = clarite.modify.rowfilter_incomplete_obs("
                               f"data={old_data_name}, "
                               f"skip={self.skip}, "
                               f"only={self.only})")

    def setup_ui(self):
        self.setWindowTitle(f"Rowfilter: Incomplete Observations")
