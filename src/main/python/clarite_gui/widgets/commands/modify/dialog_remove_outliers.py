...
from PyQt5.QtWidgets import QDialog, QLabel, QFormLayout, QDialogButtonBox, QPushButton, QLineEdit, QComboBox
...
class RemoveOutliersDialog(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.method_options_display = ["Gaussian", 'Interquartile Range (IQR)']  # Options as displayed
        self.method_options_use = ["gaussian", "iqr"]  # Options as input into the function
    ...
    def get_func(self):
        ...
        # Build the function
        def f():
            result = clarite.modify.remove_outliers(data=data, method=method, cutoff=cutoff,
                                                    skip=skip, only=only)
    ...
    def log_command(self):
        ...
        self.appctx.log_python(f"{new_data_name} = clarite.modify.remove_outliers("
    ...
    def submit(self):
        if self.data_name is not None and self.data_name in [d.name for d in self.appctx.datasets]:
            ...
            RunProgress.run_with_progress(progress_str="Removing outliers...",
